...
async removeGlobalTag(familyName, tag) {
    ...
    if (r.isConfirmed) {
        ...
        async openAddToSelection() {
            if (!this.currentSelectionId) return;
            const sel = this.currentSelectionObj;
            if (!sel) return;
            const existingIds = sel.contactIds || [];

            // Afficher une liste des contacts pas encore dans la sélection
            const available = this.filteredContacts.filter(c => !existingIds.includes(c.id));
            if (!available.length) return Swal.fire('Info', 'Tous les contacts sont déjà dans cette sélection.', 'info');

            const options = {};
            available.forEach(c => { options[c.id] = `${c.name} — ${c.structName} (${c.structCity})`; });

            const r = await Swal.fire({
                title: 'Ajouter des contacts',
                html: `<select id="swal-contact-select" class="swal2-input" multiple size="10" style="height:220px;width:100%">
                    ${available.map(c => `<option value="${c.id}">${c.name} — ${c.structName} (${c.structCity})</option>`).join('')}
                </select><p class="text-xs text-slate-400 mt-2">Ctrl+clic pour sélection multiple</p>`,
                focusConfirm: false,
                showCancelButton: true,
                confirmButtonText: 'Ajouter',
                cancelButtonText: 'Annuler',
                preConfirm: () => {
                    const select = document.getElementById('swal-contact-select');
                    return Array.from(select.selectedOptions).map(o => o.value);
                }
            });
            if (!r.isConfirmed || !r.value.length) return;
            r.value.forEach(id => { if (!sel.contactIds.includes(id)) sel.contactIds.push(id); });
            await this.saveDB();
            Swal.fire({ title: `${r.value.length} contact(s) ajouté(s) ✓`, icon: 'success', toast: true, position: 'top-end', timer: 2000, showConfirmButton: false });
        },
